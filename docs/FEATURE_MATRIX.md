...
This matrix is the release contract for Slip 1.0. Slip is designed only for personal iPhones running stock iOS.
...
| Inspection | Identity, version, build, minimum iOS, size, main executable, encryption | App Store-encrypted executables are flagged before signing because they will not launch as ordinary sideloaded apps. |
...
| Devices | USB and paired local-network discovery | USB is preferred for large transfers; Wi-Fi falls back automatically when unavailable. |
...
| Tweak/dylib injection | Safe injection requires architecture-aware Mach-O editing, dependency validation, compatible decrypted binaries, and extensive runtime testing. | Not shipped in 1.0; Slip will not bolt on an unverified injector that produces crash-prone IPAs. |
